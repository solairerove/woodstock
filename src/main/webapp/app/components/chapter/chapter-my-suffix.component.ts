...
@Component({
    selector: 'jhi-chapter-my-suffix',
    templateUrl: './chapter-my-suffix.component.html'
})
export class ChapterMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    chapters: ChapterMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/chapter-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/chapter-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ChapterMySuffix) {
        return item.id;
    }
