...
@Component({
    selector: 'jhi-answer-my-suffix',
    templateUrl: './answer-my-suffix.component.html'
})
export class AnswerMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    answers: AnswerMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/answer-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/answer-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AnswerMySuffix) {
        return item.id;
    }
